...
export class ModificarArticulosComponent implements OnInit {
  ...
  public listarporidArticulo() {
    ...
    this.route.paramMap.subscribe((params: ParamMap) => {
      this.idDetalleArticulo = params.get('id');
      this.servicioDetalleArticulo.listarPorId(this.idDetalleArticulo).subscribe(resDetalleArticulo=>{
        this.servicioArticulo.listarPorId(resDetalleArticulo.idArticulo.id).subscribe(res => {
            this.detalleArticulo = resDetalleArticulo
  ...
  public guardar() {
    ...
    this.route.paramMap.subscribe((params: ParamMap) => {
      this.idDetalleArticulito = params.get('id');
      this.servicioDetalleArticulo.listarTodos().subscribe(resDetalleArticulosCompletos=>{
        this.servicioDetalleArticulo.listarPorId(this.idDetalleArticulito).subscribe(resDetalleArticulo=>{
          ...
          if(this.formArticulo.valid){
            this.servicioArticulo.listarTodos().subscribe(resArticulosCompletos=>{
              this.servicioArticulo.listarPorId(resDetalleArticulo.idArticulo.id).subscribe(resArticulo=>{
                resDetalleArticulosCompletos.forEach(elementDetalleArt => {
                  if(elementDetalleArt.idArticulo.id == resArticulo.id){
                ...
                if(existe == true){
                  ...
                  if(existe2 == true){
                    ...
                    detalleArticulo.id = resDetalleArticulo.id
                    detalleArticulo.codigoUnico = resDetalleArticulo.codigoUnico
                    detalleArticulo.idArticulo = resDetalleArticulo.idArticulo.id
                    detalleArticulo.idEstado = resDetalleArticulo.idEstado.id
                    detalleArticulo.idTipoActivo = tipoActivo
                    detalleArticulo.idUsuario = resDetalleArticulo.idUsuario.id
