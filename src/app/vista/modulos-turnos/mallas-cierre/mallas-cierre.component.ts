...
export class MallasCierreComponent implements OnInit {
            ...
            this.servicioAsignarTurnoVendedor.listarTodos().subscribe( res =>{
              ...
              this.servicioConsultasGenerales.listarAsignarTurnosVendedores(this.fechaActual).subscribe(resAsigTurnVende=>{
                resAsigTurnVende.forEach(element => {
                  ...
                  var horaFinal = malla1.turnoFinal.split(':')
                    ...
                    if( horaActual >= horaAsignada){
                      this.servicioHistorial.listarPorId(fechaActual2, element.idVendedor).subscribe(resHistorial=>{
                        ...
                        if(this.listaHistorial.length>=1){
                          ...
                          var horaAsignadaArray = malla1.turnoFinal.split(':')
  ...
  listaMallasCierreCompletos: any = []
  exportToExcel(): void {
    this.listaMallasCierreCompletos = []
    for (let index = 0; index < this.listaMallas.length; index++) {
      ...
      var obj = {
        ...
        "Hora Asignada": element.turnoFinal,
      ...
      this.listaMallasCierreCompletos.push(obj)
    }
    import("xlsx").then(xlsx => {
      const worksheet = xlsx.utils.json_to_sheet(this.listaMallasCierreCompletos);
      const workbook = { Sheets: { 'data': worksheet }, SheetNames: ['data'] };
      const excelBuffer: any = xlsx.write(workbook, { bookType: 'xlsx', type: 'array' });
      this.saveAsExcelFile(excelBuffer, "listaMallasCierre");
    });
  }
